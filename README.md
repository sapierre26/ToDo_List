...
* ```npm frontend``` Runs the development environment for the ``````frontend`````` in the root directory
* ```npm backend``` Runs the development environment for the ``````backend`````` in the root directory
